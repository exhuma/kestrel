...
async def _deliver_a_run(gh, runner, git, tmp_path):
    """Drive a run through refine/design/code/verify to `done` + an open
    PR — the common starting point every resume test builds on."""
    svc = _service(
        gh, runner, git,
...
async def test_resume_uses_add_worktree_existing_not_add_worktree(
    ...
    add_worktree_calls_before = len(getattr(git, "_add_worktree_calls", []))
    del add_worktree_calls_before
...
async def test_triage_selects_code_for_an_implementation_only_comment(
    ...
    run = svc.get(wid)
    assert run.steps[2].status == "done"  # code step re-ran
    assert run.steps[0].status == "done"  # refine untouched
    assert run.steps[1].status == "done"  # design untouched
...
async def test_triage_selects_design_and_it_reruns_gatelessly(
    ...
    await _wait(lambda: svc.get(wid).steps[1].deliverable == "new plan")
    await _wait(lambda: svc.get(wid).status == "done")

    run = svc.get(wid)
    assert run.steps[1].deliverable == "new plan"
    assert run.steps[0].status == "done"  # refine untouched
...
async def test_triage_selects_refine_and_the_run_re_parks_at_the_gate(
    ...
    — since refine is the one gated step on this branch — the run parks
    at the approval gate again rather than reaching `done` on its own."""
    gh, git = _FakeGitHub(body="vague issue"), _FakeGit()
    runner = _FakeRunner(SessionRegistry(), outputs=[
        *_refine_noquestions("Build a widget"),
        "<PLAN>\nplan\n</PLAN>",
    ...
    run = svc.get(wid)
    assert run.steps[0].deliverable == "Build a widget, revised"
