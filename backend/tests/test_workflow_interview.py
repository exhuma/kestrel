...
_UNDERSTANDING = "<UNDERSTANDING>Build a clear widget.</UNDERSTANDING>"
...
async def test_refine_question_visible_while_awaiting_input() -> None:
    ...
    assert svc.get(wid).steps[1].deliverable == "Build a blue widget"
...
async def test_waiver_reason_lands_in_refined_issue() -> None:
    ...
    deliverable = svc.get(wid).steps[1].deliverable
...
async def test_failed_specialist_is_retried_then_hard_capped() -> None:
    ...
    async def _round(n: int):
        await _wait(
            lambda: svc.get(wid).status == "awaiting_refine_input"
            and svc.get(wid).steps[1].refine_round == n
        )
        return parse_envelope(svc.get(wid).steps[1].deliverable or "")
