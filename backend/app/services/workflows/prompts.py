...
RECONCILE_PROMPT = (
    ...
    "fact or decision into ONE question. Test it this way: if answering "
    "one question would already answer the other, they are the SAME "
    "decision — fold them, even when the wording, vocabulary, or "
    "technical register differs across domains. Two examples: Product "
    "asking 'how should accounts be created?' and Eng asking 'is this "
    "open self-registration or a fixed/seeded set of users?' are the "
    "same decision. So are Product asking 'should this favor ease of "
    "use or visual polish?' and UX asking 'should the interface "
    "optimize for usability or for design consistency?' — a "
...
GENERATION_PROMPT = (
    ...
    "Keep it to one short sentence — this is a nudge, not an essay.\n"
...
GAP_ANALYSIS_PROMPT = (
    ...
    "exactly one task covering all of it. Each task is created as a "
    "sub-task of the original ticket, so it stays linked to the "
    "requirements document — do NOT restate the document's business "
    "framing (the problem, who it is for, why it matters) in the task "
    "body; that produces redundant, near-duplicate text between a "
    "high-altitude PRD and a low-altitude task. Reference it briefly "
    "instead (e.g. 'per the linked requirements'). EACH task's body "
    "MUST still be TECHNICALLY self-contained: assume the reader has "
    "no access to this technical-analysis summary or any sibling task, "
    "and inline whatever architecture decisions, shared "
...
GAP_ANALYSIS_CRITIC_PROMPT = (
    ...
    "business framing (the problem, who it is for, why it matters) is "
    "reachable there and should NOT be restated in the task body — do "
    "not flag a task as incomplete just because it doesn't repeat that. "
    "Your ONLY job is to catch a task that is NOT technically "
    "self-contained: could a human, with access to the linked "
    "requirements document but NO access to the technical-analysis "
    "summary or any of this task's siblings, implement it correctly "
    "from its body?\n"
    ...
    "— not for wording, style, or omitting business framing that's "
    "already on the parent ticket. Return ONLY a JSON object wrapped "
